...
namespace Oxide.Plugins
{
    ...
    class RadialMenu : RustPlugin
    {
        ...
        Dictionary<string, bool> activeUsers = new Dictionary<string, bool>(); // Value: true - command, false - MMB
        #endregion

        #region Config
        class QuickMenuConfig
        {
            [JsonProperty(isRus ? "Отслеживать нажатие СКМ для открытия радиального меню? [повышенная нагрузка]" : "Track MMB presses to open a radial menu? [increased load]")]
            public bool canMMB = true;
            [JsonProperty(isRus ? "Команда для открытия радиального меню [если пусто, то отключено]" : "The command to open the radial menu [if empty, it's disabled]")]
        ...
        IEnumerator CheckConfig()
        {
            ...
            if (config.command != string.Empty) cmd.AddChatCommand(config.command, this, "Command");
            cmd.AddChatCommand("rm", this, "Command");
        ...
        void OnPlayerInput(BasePlayer player, InputState input)
        {
            ...
            if (activeUsers.ContainsKey(player.UserIDString))
            {
                CuiHelper.DestroyUi(player, "RadialMenu.MainPanel");
                activeUsers.Remove(player.UserIDString);
                return;
            }
            else activeUsers.Add(player.UserIDString, false);
            OpenPanel(player);
        }
        ...
        void Command(BasePlayer player)
        {
            if (player != null && player.CanInteract()) 
            {
                ...
                else activeUsers.Add(player.UserIDString, true);
        ...
        void AdditionalFunctionality(BasePlayer player, ref HashSet<MenuSectionClass> validSections, string[] lvls)
        {
            ...
            switch (lvls[0])
            {
                ...
                case "_homes_all":
                    ...
                    else if (Teleportation != null && Teleportation.IsLoaded && Teleportation.Author == "OxideBro") teleportHomes = Teleportation?.Call("GetHomes", (ulong)player.userID) as Dictionary<string, Vector3>;
                    else 
                    {
                    ...
                    for (int i = 0; i < homes.subsections.Count; i++)
                    {
                        ...
                        {
                            string grid =  MapHelper.PositionToString(player.transform.position);
                ...
                case "_teammates_all":
                    ...
                    if (lvls[0].StartsWith("_friends"))
                    {
                        ...
                        friendsList = Friends?.Call<ulong[]>("GetFriends", (ulong)player.userID).ToList();
                    }
                    ...
                    friendsList?.Remove((ulong)player.userID);
