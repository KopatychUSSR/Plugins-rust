...
{
    [Info("ZetaVehicle", "rust-plug.ru", "0.1.3fix")]
...
void spawn(BasePlayer player, string prefab)
        {
            ...
            {
                if (player.modelState != null && player.modelState.waterLevel <= 0f)
                {
                    ...
                    for (int i = 0; i < 8; i++)
                    {
                        ...
                        Vector3 checkPos = player.transform.position + new Vector3(Mathf.Cos(angle) * searchRadius, 0f, Mathf.Sin(angle) * searchRadius);
                        float waterHeight = TerrainMeta.WaterMap.GetHeight(checkPos);
                        float groundHeight = TerrainMeta.HeightMap.GetHeight(checkPos);
                        
                        if (waterHeight > groundHeight)
                        {
                            spawnpos = new Vector3(checkPos.x, waterHeight + 0.5f, checkPos.z);
                            foundWater = true;
                            break;
                        }
                    }
                    ...
                    if (!foundWater)
                    {
                        ...
                        if (!foundWater)
                        {
                            UiMSG(player, config.messages[18]);
                            return;
                        }
                    }
                }
                ...
                {
                    spawnpos = new Vector3(vector.x, TerrainMeta.WaterMap.GetHeight(vector) + 0.5f, vector.z);
                }
                
                if (spawnpos.y < -100f || float.IsNaN(spawnpos.y))
                {
                    spawnpos.y = Mathf.Max(TerrainMeta.HeightMap.GetHeight(spawnpos), TerrainMeta.WaterMap.GetHeight(spawnpos)) + 0.5f;
            ...
            // Final validation of spawn position
            if (float.IsNaN(spawnpos.x) || float.IsNaN(spawnpos.y) || float.IsNaN(spawnpos.z) || 
                float.IsInfinity(spawnpos.x) || float.IsInfinity(spawnpos.y) || float.IsInfinity(spawnpos.z) ||
                spawnpos.y < -100f)
            {
                UiMSG(player, "НЕВОЗМОЖНО ОПРЕДЕЛИТЬ ПОДХОДЯЩЕЕ МЕСТО ДЛЯ СПАВНА!");
                return;
            }
            ...
            if (entity is Minicopter)
            {
                ...
                foreach (var z in entity.GetComponent<Minicopter>().children)
                {
            ...
            else if (entity is BaseBoat)
            {
                ...
                bool isFuelHandled = false;
                
                // Handle specific boat types
                if (boat is MotorRowboat motorRowboat)
                {
                    if (che.infinitefuel) 
                        motorRowboat.fuelPerSec = 0;
                        
                    // Ensure the boat is properly positioned at the water level
                    Vector3 boatPos = motorRowboat.transform.position;
                    float waterHeight = TerrainMeta.WaterMap.GetHeight(boatPos);
                    if (boatPos.y < waterHeight)
                        motorRowboat.transform.position = new Vector3(boatPos.x, waterHeight + 0.5f, boatPos.z);
                }
                else if (boat is RHIB rhib)
                {
                    if (che.infinitefuel) 
                        rhib.fuelPerSec = 0;
                        
                    // Ensure the RHIB is properly positioned at the water level
                    Vector3 boatPos = rhib.transform.position;
                    float waterHeight = TerrainMeta.WaterMap.GetHeight(boatPos);
                    if (boatPos.y < waterHeight)
                        rhib.transform.position = new Vector3(boatPos.x, waterHeight + 0.5f, boatPos.z);
                }
                
                // Find and add fuel to the boat
                foreach (var z in boat.children)
                {
                    if(z.prefabID == 198420611 || z.prefabID == 1394312733)
                    {
                        givefuel((z as StorageContainer).inventory, che.fuel, che.infinitefuel);
                ...
                // If we couldn't find the fuel container by prefabID, try finding it by type
                if (!isFuelHandled)
                {
                    foreach (var child in boat.children)
                    {
                        var container = child.GetComponent<StorageContainer>();
                        if (container != null && container.inventory != null)
                        {
                            givefuel(container.inventory, che.fuel, che.infinitefuel);
                            break;
                        }
                    }
                }
