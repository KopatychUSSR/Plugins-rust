                ...
                switch (GetSettings().CurrentType)
                {
                    ...
                    case HitType.Text:
                    {
                        ...
                        if (target is BasePlayer)
                        {
                            ...
                            if (info.isHeadshot)
                                textDamage = $"<color=#FF8C00>{textDamage}</color>";
                            if (targetPlayer.IsWounded())
                            {
                                textDamage = "<color=#FF6347>УПАЛ</color>";
                                if (info.isHeadshot)
                                    textDamage += " <color=#FF6347>ГОЛОВА</color>";
                            }
        
                            if (Player.currentTeam == targetPlayer.currentTeam && Player.currentTeam != 0)
                            {
                                textDamage = "<color=#FF8C00>ДРУГ</color>";
                                division = 1;
                            }
                        }
                        ...
                        container.Add(new CuiElement()
                        {
                            ...
                            Components =
                            {
                                new CuiTextComponent { Text = $"<b>{textDamage}</b>", Color = HexToRustFormat("#FF8C00"), Font = "robotocondensed-bold.ttf", FontSize = (int) Mathf.Lerp(15, 30, division), Align = TextAnchor.MiddleCenter, },
        ...
        private void UI_DrawInterface(BasePlayer player)
        {
            ...
            for (int i = 0; i < 6; i += 3)
            {
                ...
                foreach (var check in Settings.HitSettings.Skip(i).Take(3))
                {
                    ...
                    string color = "1 0.3 0.2 0.12";
                    if (obj.GetSettings().CurrentType == check.Key)
                    {
                        text = "МАРКЕР ВЫБРАН";
                        color = "1 0.3 0.2 0.12";  
                    }
