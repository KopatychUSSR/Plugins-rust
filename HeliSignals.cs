...
1.2.31
 - Fixed: Hackable locked crates not spawning when set in config

1.2.30
 - Fixed: Wave discord announcements not being correct in some cases
 - Fixed: Heli Signals interfering with Hackable crates from other plugins
 - Fixed: Heli fleeing from player in some cases when shouldn't
 - Fixed: Occasional Harmony patching error
 - Fixed: Heli sometimes getting stuck in low aggro orbit state

1.2.29
 - Fixed: Wave helicopters spawning before previous wave ended
 - Fixed: Cooldowns not working properly in some cases
 - Fixed: Discord embed not having Heli HP info on called
 - Fixed: Heli moving away from calling player and not returning to normal behaviour
 - Fixed: If moving away, heli will now return when hit by auth'd player
 - Added: Heli crate despawn timer config option

1.2.28
 - Fixed: Refunding waves giving incorrect non-wave signal

1.2.27
 - Fixed: Loot item dupes in certain cases when dupes allowed set to false
 - Fixed: Support for BetterNPC now fixed (when false now prevents Better NPC spawn)
 - Fixed: HackableLockedCrate config hack time not working in certain cases
 - Fixed: Heli crates falling on Unloadable Train wagons not being lootable
 - Fixed: Heli targeting players under water, now they do not!
 - Fixed: Heli glitching under terrain and ocean
 - Fixed: Player calling location/heli killed grid position accuracy
 - Fixed: Team kill messages only going to the called player
 - Fixed: Players exploiting base protection by fighting on non-team mebers bases when auth'd on TC
 - Added: Config option to allow/prevent Epic Loot adding items to crates
 - Added: Option to make heli and hackable crates float if fall in water
 - Added: Config option for minimum heli altitude
 - Added: Config option for maximum heli altitude (prevents raised flight path with 7th June FP update)
 - Changed: Code clean up & performance/efficiency improvements++
 - Added: New Discord embed announcements with language entries
...
{
    [Info("Heli Signals", "ZEODE", "1.2.32")]
    ...
    public class HeliSignals : RustPlugin
    {
        ...
        private static Harmony harmony = new Harmony("HeliSignals");
        ...
        private void Unload()
        {
            ...
            foreach (var netId in heliDataCopy.Keys)
            {
                ...
                {
                    PrintError($"Error cleaning up Heli: {ex.Message}");
                }
            }
            ...
            try
            {
                harmony?.UnpatchAll("HeliSignals");
                PrintWarning("INFO: Harmony patches removed and cache cleared successfully!");
            }
            catch (System.Exception ex)
            {
                PrintError($"ERROR during removing HeliSignals Harmony patches!");
            }
            Instance = null;
        }
        ...
        private void OnLootSpawn(LootContainer lootContainer)
        {
            timer.Once(2f, () =>
            {
                ...
                {
                    PrintError($"Error in OnLootSpawn timer: {ex.Message}");
                    PrintError($"StackTrace: {ex.StackTrace}");
                }
            });
        }
        ...
        object CanEntityTakeDamage(BaseCombatEntity entity, HitInfo hitInfo)
        {
            ...
            return HeliProfileCache.ContainsKey(skinId) ? true : (object)null;
        }
        ...
        object OnBotReSpawnPatrolHeliKill(PatrolHelicopterAI heliAi)
        {
            ...
            return HeliProfileCache.ContainsKey(heliAi.helicopterBase.skinID) ? true : (object)null;
        }
        ...
        private void LoadProfileCache()
        {
            ...
            foreach (var key in _heliConfig.Keys)
            {
                ...
                {
                    PrintError($"ERROR: Duplicate 'Profile Shortname' for HeliProfile: {key} with Profile Shortname: {_heliConfig[key].GiveItemCommand}, these must be unique for each profile. Correct your config & reload.");
                    continue;
                }
                else
                {
                ...
                {
                    PrintError($"ERROR: Duplicate 'SkinID' for HeliProfile: {key} with SkinID: {_heliConfig[key].SignalSkinID}, these must be unique for each profile. Correct your config & reload.");
                    continue;
                }
                else
                {
            ...
            foreach (var key in _waveConfig.Keys)
            {
                ...
                {
                    PrintError($"ERROR: Duplicate 'Profile Shortname' for WaveProfile: {key} with Profile Shortname: {_waveConfig[key].GiveItemCommand}, these must be unique for each profile. Correct your config & reload.");
                    continue;
                }
                else
                {
                ...
                {
                    PrintError($"ERROR: Duplicate 'SkinID' for WaveProfile: {key} with SkinID: {_waveConfig[key].SkinId}, these must be unique for each profile. Correct your config & reload.");
                    continue;
                }
                else
                {
        ...
        // Public helper for Harmony or other external classes to log/debug
        ...
        private void ApplyHarmonyPatches()
        {
            try
            {
                PrintWarning($"====== Harmony Patching Begin ======");

                harmony.UnpatchAll("HeliSignals");

                var updateTargetListMethod = typeof(PatrolHelicopterAI).GetMethod("UpdateTargetList",
                    System.Reflection.BindingFlags.Instance |
                    System.Reflection.BindingFlags.Public |
                    System.Reflection.BindingFlags.NonPublic);

                PrintWarning($"INFO: PatrolHelicopterAI.UpdateTargetList method found: {updateTargetListMethod != null}");

                var fireRocketMethod = typeof(PatrolHelicopterAI).GetMethod("FireRocket",
                    System.Reflection.BindingFlags.Instance |
                    System.Reflection.BindingFlags.Public |
                    System.Reflection.BindingFlags.NonPublic);

                PrintWarning($"INFO: PatrolHelicopterAI.FireRocket method found: {fireRocketMethod != null}");

                int patchCount = 0;
                
                if (updateTargetListMethod != null)
                {
                    harmony.Patch(updateTargetListMethod, prefix: new HarmonyMethod(typeof(PatrolHeliUpdateTargetListPatch), "Prefix"));
                    PrintWarning($"INFO: ✓ Patched: PatrolHelicopterAI.UpdateTargetList");
                    patchCount++;
                }
                else
                {
                    PrintError("ERROR: PatrolHelicopterAI.UpdateTargetList method not found!");
                }

                if (fireRocketMethod != null)
                {
                    harmony.Patch(fireRocketMethod, prefix: new HarmonyMethod(typeof(PatrolHeliFireRocketPatch), "Prefix"));
                    PrintWarning($"INFO: ✓ Patched: PatrolHelicopterAI.FireRocket");
                    patchCount++;
                }
                else
                {
                    PrintError("ERROR: PatrolHelicopterAI.FireRocket method not found!");
                }

                PrintWarning($"INFO: Successfully applied {patchCount} Harmony patches!");
                PrintWarning($"====== Harmony Patching Complete ======");
            }
            catch (System.Exception ex)
            {
        ...
        public class HeliSignalComponent : MonoBehaviour
        {
            ...
            public bool SignalAborted()
            {
                ...
                if (config.heli.playerLimit > 0 && !Instance.permission.UserHasPermission(player.UserIDString, permAdmin))
                {
                    ...
                    {
                        var heli = BaseNetworkable.serverEntities.Find(new NetworkableId(netId)) as PatrolHelicopter;
                        if (heli != null && heli.OwnerID == player.userID)
                            heliCount++;
                    }
        ...
        public class HeliComponent : MonoBehaviour
        {
            ...
            private HeliData _heliProfile;
            private HeliData _nextHeliProfile;
            ...
            private void AddHeliToWaveTracking(PatrolHelicopter newHeli)
            {
                ...
                if (isWaveHeli)
                {
                    ...
                    {
                        Instance.PrintError($"Wave tracking error: nextWaveTime {nextWaveTime} not found in WavesCalled dictionary");
                    }
                }
            }
            ...
            public void CallNextWaveHeli(string nextHeliProfile)
            {
                ...
                {
                    Instance.PrintError($"ERROR: No such profile: {nextHeliProfile}, check config.");
                    return;
                }
